...
export async function handleSearchArchive(
  ...
  try {
    ...
    // When results are sparse, check whether unenriched conversations exist
    if (mapped.length < limit) {
      const row = db.prepare('SELECT COUNT(*) as n FROM conversations WHERE enriched = 0').get() as { n: number }
      if (row.n > 0) {
        return {
          results: mapped,
          _note: `${row.n} conversation(s) are still being enriched and won't appear in search results yet. Use get_conversation({ conversation_id }) for direct access to any conversation by ID.`,
        }
      }
    }
